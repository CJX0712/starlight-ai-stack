...
| 单次问答端到端 | 3.7–13.0s（含检索与生成） |
| 六模块自检 | 6/6 通过 |
| 单元测试 | 14/14 通过（不依赖模型服务） |
| 端到端 | 3 个问题全部命中预期片段，引用合法，幂等重复摄取新增 0 |

> 问答耗时从最初的约 100s/问 降到 3.7–13s/问，关键改动是「助手预填充」，见 [ADR-005](docs/ADR-005-助手预填充.md)。
